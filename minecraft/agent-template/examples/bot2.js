...
const botState = {
  bot: process.env.BOT_NAME || 'ArenexBot2',
...
function initBot() {
  bot = mineflayer.createBot({
    ...
    username: process.env.BOT_NAME || 'ArenexBot2',
    version: '1.20.1'
  });
  ...
  bot.once('spawn', () => {
    ...
    try {
      const { startWoodCollector } = require('../src/strategies/woodCollector');
      if (typeof startWoodCollector === 'function') {
        startWoodCollector(bot, botState);
      }
    } catch (e) {
      console.log('No default strategy detected, idling. Error:', e.message);
    }
  });
...
app.post('/stop', (req, res) => {
  ...
  botState.current_action = ACTIONS.FINISHED;
  if (bot) {
    try { bot.quit(); } catch (e) { console.error(e.message); }
  }
