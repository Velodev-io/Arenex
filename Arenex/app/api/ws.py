...
async def match_websocket(websocket: WebSocket, match_id: int):
    ...
    try:
        ...
        try:
            ...
            pass
@router.websocket("/{match_id}/minecraft")
async def minecraft_websocket(websocket: WebSocket, match_id: str):
    await websocket.accept()
    logger.info(f"ACCEPTED Minecraft WS connection for match {match_id}")
    ...
    try:
        while True:
            ...
            if message:
                ...
                # No catchup for Minecraft yet, just stream live
                data = json.loads(message["data"])
                if data.get("type") == "match_end":
                    break
    except WebSocketDisconnect:
        logger.info(f"Spectator disconnected from Minecraft match {match_id}")
    except Exception as e:
        logger.error(f"Minecraft WebSocket error in match {match_id}: {e}")
    finally:
        await pubsub.unsubscribe(channel)
        await pubsub.close()
        try:
