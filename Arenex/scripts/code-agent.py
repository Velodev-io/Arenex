...
class CodeAgent:
    ...
    def check_git_status(self):
        if self.mode == 'fix' and self.target_path == '.' and self.repo and self.repo.is_dirty():
            msg = "[bold red]ERROR: Uncommitted changes detected. Commit or stash before running --fix mode.[/bold red]"
            if console:
                console.print(msg)
            else:
                print(msg)
    ...
    def apply_fixes(self):
        ...
        if self.repo and self.target_path == '.':
            ...
            msg = f"[green]Fixes applied on branch {branch}[/green]"
            if console:
                console.print(msg)
            else:
                print(msg)
        ...
        if not (self.repo and self.target_path == '.'):
             msg = f"[green]Applied {self.fixed_count} fixes locally.[/green]"
             if console:
                 console.print(msg)
             else:
                 print(msg)
...
def main():
    ...
    if RICH_AVAILABLE and Table:
        ...
        print("\nArenex Code Agent Results:")
        for s in ['critical', 'high', 'medium', 'low']:
            print(f"{s.upper()}: {len([i for i in agent.issues if i['severity'] == s])}")

    msg = f"Fixed: {agent.fixed_count} | Manual: {len([i for i in agent.issues if not i['auto_fixed']])}"
    if console:
        console.print(msg)
    else:
        print(msg)
