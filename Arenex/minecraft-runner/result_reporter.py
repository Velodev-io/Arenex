    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.post(
                f"{PLATFORM_API_URL}/matches/{match_id}/live-viewer",
                json={"had_live_viewer": True},
        ...
        logger.error(f"Failed to mark live viewer for match {match_id}: {e}")
...
async def report_result(
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(f"{PLATFORM_API_URL}/matches/result", json=payload)
